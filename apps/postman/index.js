    ...
    methods: {
            ...
            if ('serviceWorker' in navigator) {
                window.addEventListener('load', () => {
                    navigator.serviceWorker.register('./sw.js')
                        .then((registration) => {
                            console.log('已启用 XFeHelper Mock Server 已注册:', registration.scope);
                            // 通知Vue组件Mock服务器已就绪
        ...
        getEffectiveContentTypeForAi() {
            ...
            if (this.methodContent.toLowerCase() === 'post' && this.urlencodedDefault) {
                return 'application/x-www-form-urlencoded（XFeHelper POST 默认携带）';
            }
            return '未设置';
        },
        ...
        askAiForRequest() {
            ...
            runInlineToolAi(this.aiPanel, {
                ...
                instruction: [
                    '请作为 XFeHelper 简易 Postman 的接口调试助手，基于当前请求现场给出可执行排查方案。',
        ...
        askAiForPostman() {
            ...
            runInlineToolAi(this.aiPanel, {
                ...
                instruction: [
                    '请基于 XFeHelper 简易 Postman 的请求和响应现场做接口调试诊断。',
        ...
        setDemo: function (type) {
            if (type === 1) {
                ...
                this.paramContent = JSON.stringify({
                    title: '测试数据',
                    content: '这是一个通过XFeHelper Mock服务器创建的测试数据',
                    category: 'test',
