...
function setInlineAiGuide(state, options = {}) {
    ...
    Object.assign(state, {
        ...
        title: options.title || 'XFeHelper AI',
...
function clipTextForPrompt(text, limit = MAX_CONTEXT_LENGTH) {
    ...
    return `${head}\n\n[XFeHelper 已截断中间 ${value.length - head.length - tail.length} 个字符]\n\n${tail}`;
}
...
function buildToolAiMessages(toolKey, request = {}) {
    ...
    const userContent = [
        `# ${request.title || (pack && pack.title) || 'XFeHelper AI 任务'}`,
...
async function runInlineToolAi(state, request = {}) {
    ...
    Object.assign(state, {
        ...
        title: request.title || 'XFeHelper AI',
