...
let BgPageInstance = (function () {
    ...
    let _notifyToolOpenFailure = function (tool, error) {
        ...
        console.error('[XFeHelper] DynamicToolRunner failed:', message, error);
        notifyText({
            title: '工具打开失败',
            message,
            autoClose: 5000
    ...
    let _updateBrowserAction = function (action, showTips, menuOnly) {
        ...
        if (showTips) {
            ...
            switch (action) {
                case 'install':
                    actionTxt = '工具已「安装」成功，并已添加到弹出下拉列表，点击XFeHelper图标可正常使用！';
                    break;
                case 'offload':
            ...
            switch (reason) {
                ...
                case 'update':
                    ...
                    if (previousVersion === '2019.12.2415') {
                        notifyText({
                            message: '历尽千辛万苦，XFeHelper已升级到最新版本，可以到插件设置页去安装旧版功能了！',
                            autoClose: 5000
                        });
                    }
                    break;
    ...
    function fetchHotfixJson(callback) {
        ...
        fetch(url)
            ...
            .catch(error => {
                console.warn('[XFeHelper] hotfix.json 获取失败:', url, error && error.message);
    ...
    function checkAndFetchPatchs() {
        ...
        chrome.storage.local.get(STORAGE_KEY, (result) => {
            ...
            if (now - lastCheck > PATCH_CHECK_INTERVAL) {
                console.log(`[XFeHelper] 距离上次检查已超过5min，开始检查热更新...`);
                
                fetchFehelperPatchs((result) => {
                    if (result && result.success) {
                        console.log(`[XFeHelper] 自动热更新成功，版本: v${result.version}`);
                    } else if (result && result.notFound) {
                        console.log(`[XFeHelper] 当前版本暂无热更新补丁`);
                    } else {
                        console.log(`[XFeHelper] 自动热更新检查失败:`, result?.error);
                    }
                ...
                console.log(`[XFeHelper] 距离上次检查不足5min，下次检查时间: ${nextCheck.toLocaleString()}`);
            }
        });
    }
                    ...
                    return fetch(`${patchUrl}?t=${Date.now()}`)
                        ...
                        .then(data => {
                            ...
                            chrome.storage.local.set(storageData, () => {
                                console.log(`[XFeHelper] 成功获取版本 v${version} 的热修复补丁`);
                                callback && callback({ success: true, version });
                            });
                        });
                } else {
                    // 文件不存在
                    console.log(`[XFeHelper] 服务器上不存在版本 v${version} 的补丁文件`);
                    callback && callback({ success: false, error: '补丁文件不存在', notFound: true });
                }
            })
            .catch(e => {
                console.warn('[XFeHelper] 热修复补丁获取失败:', patchUrl, e && e.message);
