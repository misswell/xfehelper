/**
 * Json Page Automatic Format Via XFeHelper
 * @author zhaoxianlie
 */
...
window.JsonAutoFormat = (() => {
    ...
    let _getHtmlFragment = () => {
        ...
        return [
            ...
            '            <img src="' + chrome.runtime.getURL('static/img/fe-16.png') + '" alt="XFeHelper"/><span>XFeHelper</span></a>' +
        ...
        if (jsonObj != null && typeof jsonObj === "object") {
            ...
            if (formatOptions['MAX_JSON_KEYS_NUMBER']) {
                ...
                if (keysCount > formatOptions['MAX_JSON_KEYS_NUMBER']) {
                    let msg = '当前JSON共 <b style="color:red">' + keysCount + '</b> 个Key，大于预设值' + formatOptions['MAX_JSON_KEYS_NUMBER'] + '，已取消自动格式化；可在XFeHelper设置页的「JSON 自动格式化 Key 数上限」中调整此配置！';
                    return toast(msg);
                }
            }
