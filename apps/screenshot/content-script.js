...
window.screenshotContentScript = function () {
     ...
     */
    function showXFeHelperUI(originalDisplays) { // Accept argument
    ...
    function cleanup() {
        ...
        // 恢复UI显示 - 传递存储的原始状态
        showXFeHelperUI(window._fh_original_displays || {});
        showFixedElements();
 ...
 // console.log('开始执行截图，模式：' + params.captureType);
        ...
        try {
            ...
            container.innerHTML = `
                <div id="fehelper_screenshot" style="position:fixed;left:0;top:0;right:0;z-index:1000000;padding:15px;background:rgba(0,0,0,0.8);color:#fff;text-align:center;">
                    <h3 style="margin:0 0 10px 0;font-size:16px;">XFeHelper 网页截图工具</h3>
            ...
            // 确保DOM已准备好
            if (!document.body) {
                // console.error('XFeHelper截图：document.body不存在，无法添加截图UI');
                // 尝试等待DOM加载完成
            ...
            // console.log('XFeHelper截图UI已添加到页面');
            
        } catch (error) {
            // console.error('XFeHelper截图UI创建失败：', error);
    ...
    function buildFilenameFromUrl() {
        ...
        return 'XFeHelper' + name + '-' + Date.now() + '.png';
    }
